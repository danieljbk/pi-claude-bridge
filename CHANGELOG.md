...
- **New: subscription usage in the pi footer** — the five-hour and weekly windows Claude Code streams in every `rate_limit_event` are folded into one status line under pi's token line, `Claude 5h 2% (resets 7:00pm) · week 97% (resets Sun 3:00am)`, with an `extra usage in use` flag when overage credits are being spent. Claude Code 2.1.263 attaches both windows (`unifiedWindows`) to every event; an older build's single-window frame is recorded the same way, and a window is shown only once a sample for it has arrived. No credentials are read and no request is made.
