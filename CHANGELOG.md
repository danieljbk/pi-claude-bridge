...
- **Fix: a custom pi `--system-prompt` now replaces Claude Code's preset on the provider path** — it was appended after the preset, so a session that replaced pi's prompt to shed guidance still carried the preset's ten thousand characters, and a floor session could not get below about 4,800 tokens of prefix. A custom prompt is now sent as the system prompt string, and Claude Code adds only its one identity line; measured 1,639 tokens of prefix for the same turn. Sessions without a custom prompt are unchanged: the preset stays and pi's context files, skills and `--append-system-prompt` ride after it.
- **New: subscription usage on pi's footer line** — the five-hour and weekly windows Claude Code streams in every `rate_limit_event` are shown beside the token counts, in pi's own terse form and ink: `↑2 ↓33 R22k W292 CH98.7% 2.2%/1.0M 5h61% 7d9%`, each window coloured like the context percentage (warning past 70%, error past 90%). The bridge replaces the footer with a copy of pi's that carries the extra segment, so the model stays right-aligned and gives way first as the width shrinks; the right side is cut to the same register, `fable-5-1 high` in place of `(claude-bridge) claude-fable-5-1 • high`, with the provider named only when the active model is not the bridge's. Claude Code 2.1.263 attaches both windows (`unifiedWindows`) to every event; an older build's single-window frame is recorded the same way, and a window is shown only once a sample for it has arrived. No credentials are read and no request is made.
