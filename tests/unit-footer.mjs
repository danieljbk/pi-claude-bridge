...
describe("the right side", () => {
	it("keeps the family alone and shortens the thinking level", () => {
		assert.equal(shortModelId("claude-fable-5-1"), "fable");
		assert.equal(shortModelId("claude-haiku-4-5-20251001"), "haiku");
...
describe("renderFooter", () => {
	...
	it("puts the usage on the stats line after the context, terse, five-hour first", () => {
		...
		assert.match(lines[1], /^↑2 ↓33 R22k W292 CH98\.7% 2\.2%\/1\.0M 5h61% 7d9% +fable high$/);
	});
	...
	it("shows no usage segment before a sample has arrived", () => {
		...
		assert.match(lines[1], /2\.2%\/1\.0M +fable high$/);
	});
	...
	it("names the provider only when the model is not the bridge's, when there is room, and drops it first", () => {
		...
		assert.match(onBridge[1], / {2,}fable high$/);
		...
		assert.match(wide[1], /\(anthropic\) fable high$/);
		const narrow = renderFooter(62, theme, two, { ctx: () => other, usage: () => usageWith(0.61, 0.09) });
		assert.doesNotMatch(narrow[1], /\(anthropic\)/);
		assert.match(narrow[1], /5h61% 7d9% +fable high$/);
	});
	...
	it("cuts the model from the right before touching the stats, then the stats last", () => {
		...
		// The stats are 45 columns; at 51 the model has 4 left and is cut to fit.
		assert.equal(at(51), "↑2 ↓33 R22k W292 CH98.7% 2.2%/1.0M 5h61% 7d9%  fabl");
