...
import { renderFooter, formatTokens } from "../src/footer.js";
...
describe("renderFooter", () => {
	...
	it("puts the usage on the stats line after the context, terse, five-hour first", () => {
		...
		assert.match(lines[1], /^↑2 ↓33 R22k W292 CH98\.7% 2\.2%\/1\.0M 5h61% 7d9% +claude-fable-5-1 • high$/);
	});
	...
	it("shows no usage segment before a sample has arrived", () => {
		...
		assert.match(lines[1], /2\.2%\/1\.0M +claude-fable-5-1 • high$/);
	});

	it("prefixes the provider when several are available and there is room, and drops it first", () => {
		const two = footerData({ getAvailableProviderCount: () => 2 });
		const wide = renderFooter(120, theme, two, { ctx: () => ctx, usage: () => usageWith(0.61, 0.09) });
		assert.match(wide[1], /\(claude-bridge\) claude-fable-5-1 • high$/);
		const narrow = renderFooter(72, theme, two, { ctx: () => ctx, usage: () => usageWith(0.61, 0.09) });
		assert.doesNotMatch(narrow[1], /\(claude-bridge\)/);
		assert.match(narrow[1], /5h61% 7d9%/);
	});
	...
	it("cuts the model from the right before touching the stats, then the stats last", () => {
		...
		// The stats are 45 columns; at 60 the model has 13 left and is cut to fit.
		assert.equal(at(60), "↑2 ↓33 R22k W292 CH98.7% 2.2%/1.0M 5h61% 7d9%  claude-fable-");
