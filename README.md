...
**Usage in the footer:** every request Claude Code makes streams a `rate_limit_event` carrying your subscription's rolling windows, and the bridge shows them on pi's stats line beside the token counts, in pi's own form: `↑2 ↓33 R22k W292 CH98.7% 2.2%/1.0M 5h61% 7d9%`. Each window is coloured like the context percentage, warning past 70% and error past 90%. The segment appears after the first request of the session and updates per request; nothing is fetched and no credentials are read. To draw it there the bridge replaces pi's footer with a copy that carries the segment, so a footer another extension sets will be overridden, and other extensions' `setStatus` texts still show on the line below.
